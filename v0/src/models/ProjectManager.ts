...
// Define IProjectManager interface
...
// Define User Schema
const userSchema = new Schema<IProjectManager>(
  {
    email: {
      type: String,
...
userSchema.pre("save", async function (next) {
  const user = this as IProjectManager;
...
// Export IProjectManager Model
const IProjectManager =
  models?.IProjectManager ||
  model<IProjectManager>("IProjectManager", userSchema, "register_user");
